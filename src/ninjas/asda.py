...
def extract_tesco() -> None:
    """Extract Asda groceries."""
    print("Extracting Asda items")
    main_urls = \
...
easter/all?sortBy=relevance&count=40"]
    ...
    products = asda_ize(parse_many(pages, parse_one))
    return products

def asda_ize(products:list)->list:
    """Convert the products to Asda format."""
    ...
    for product in products:
        ...
        standardized_products.append({"name": name, "price": price, \
             "price_relative": relative_price, "units":units, \
                "supermarket":"Asda", "link":product["link"], "img":product["img"]})

    return standardized_products

def parse_one(html:str|None)->List:
    """Extract product details from the Asda HTML content"""
    if not html:
        return []
...
def extract_n_pages(html):
    """Extract the number of pages from the Asda html."""
    soup = BeautifulSoup(html, "html.parser")
